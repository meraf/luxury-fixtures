...
const uploadDirectToCloudinary = async (file: File): Promise<string> => {
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 2: Send JSON to your database
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 text-slate-900 font-sans relative">
      ...
      <div className="bg-white p-1.5 border border-slate-200 rounded-2xl shadow-sm flex space-x-2 max-w-md">
        ...
        >
          <RefreshCw className="w-4 h-4 mr-2" /> Stock Warehouse
        </button>
      </div>

      {/* --- FORM A: INITIALIZE ASSET RECORD --- */}
      {mode === 'create' && (
        <form onSubmit={handleCreateProduct} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-4 flex flex-col justify-between">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Opening Storage Inventory Count</label>
                <input 
      ...
      {/* --- FORM B: ADD LEVEL STOCKS (PATCH) --- */}
      {mode === 'restock' && (
        <form onSubmit={handleRestockWarehouse} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              ...
              >
                ...
                {products.map(p => (
                  <option key={p.id} value={p.id}>{p.name} [{p.sku}] — (In Stock: {p.warehouseStock})</option>
                ))}
              </select>
            </div>
